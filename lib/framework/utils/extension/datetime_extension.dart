...
extension DateTimeExtension on DateTime {
  String toStringWithCustomDate(String outputFormat) {
    return DateFormat(outputFormat).format(toLocal());
  }

  String? formatDateIfBeforeToday() {
    // Get current date (ignoring time) and compare it with the input date
    ...
    DateTime inputDateOnly = DateTime(year, month, day);

    // If the input date is before today, return it in dd MM yyyy format
    if (inputDateOnly.isBefore(currentDateOnly)) {
      final DateFormat formatter = DateFormat('d MMMM yyyy');
      // Log the formatted date
      print('Formatted Date: ${formatter.format(this)}');
      return formatter.format(this);
    } else {
      return null; // Return null if the given date is not before today
    }
  }
