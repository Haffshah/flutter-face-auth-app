...
class TextMask extends TextInputFormatter {
  ...
  String getMaskedText(String newTxt, {String oldText = ''}) {
    ...
    for (int i = 0; i < pallet.length; i++) {
      if (pallet[i] != '#') {
        if (tempIndex == i - 1) {
          list.last = <int, String>{list.last.keys.first: list.last.values.first + pallet[i]};
        } else {
    ...
    tempIndex = list.indexWhere(
        (Map<int, String> element) => oldText.endsWith(element.values.first) || newTxt.endsWith(element.values.first));
